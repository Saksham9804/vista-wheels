...
export default function CustomerSignup() {
  ...
  const validateStep = (s: number): boolean => {
    ...
    if (s === 1) {
      ...
      if (!formData.postalCode || !/^[0-9]{6}$/.test(formData.postalCode))
        errs.postalCode = "Valid 6-digit postal code required";
      ...
      if (!formData.emergencyContactPhone || !/^[0-9]{10}$/.test(formData.emergencyContactPhone))
        errs.emergencyContactPhone = "Valid 10-digit phone required";
    } else if (s === 4) {
  ...
  const documentsUploaded = [files.licenseFront, files.licenseBack, files.aadharFront, files.aadharBack].filter(
    Boolean,
  ).length;
  ...
  const maskedAadhar = formData.aadharNumber ? "XXXX XXXX " + formData.aadharNumber.slice(-4) : "";
          ...
          <div className="mb-6">
            <div className="flex items-center justify-between text-sm mb-2">
              <span className="text-muted-foreground">
                Step {step} of 6 — {signupSteps[step - 1].name}
              </span>
                        ...
                        <div className="grid grid-cols-2 gap-1 text-xs">
                          {passwordRequirements.map((req) => (
                            <div key={req.label} className="flex items-center gap-1">
                              ...
                              <span
                                className={req.test(formData.password) ? "text-green-500" : "text-muted-foreground"}
                              >
                                {req.label}
                              </span>
                            </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <select
                        ...
                        onChange={handleChange}
                        className={`w-full h-10 px-3 rounded-md border bg-background text-sm ${errors.gender ? "border-destructive" : "border-input"}`}
                      >
                ...
                <>
                  <div className="space-y-2">
                    ...
                    {errors.drivingLicenseNumber && (
                      <p className="text-sm text-destructive">{errors.drivingLicenseNumber}</p>
                    )}
                  </div>
                  <div className="space-y-2">
                    <Label>
                    ...
                    <Input
                      ...
                      onChange={handleChange}
                      className={errors.drivingLicenseExpiry ? "border-destructive" : ""}
                    />
                    {errors.drivingLicenseExpiry && (
                      <p className="text-sm text-destructive">{errors.drivingLicenseExpiry}</p>
                    )}
                  </div>
                      ...
                      {[
                        {
                          ...
                          done: !!formData.fullName && !!formData.email && !!formData.phone,
                        },
                        { label: "Phone verified", done: phoneVerified },
                        {
                          label: "Address & emergency contact",
                          done: !!formData.addressLine1 && !!formData.emergencyContactName,
                        },
                    ...
                    <div className="bg-card rounded-xl border border-border p-4 space-y-3 text-sm">
                      <div className="grid grid-cols-2 gap-2">
                        ...
                        <div>
                          ...
                          <span className="font-medium">{formData.drivingLicenseNumber}</span>
                        </div>
                        <div>
                          <span className="text-muted-foreground">Aadhar:</span>{" "}
                          <span className="font-medium">{maskedAadhar}</span>
                        </div>
                        <div>
